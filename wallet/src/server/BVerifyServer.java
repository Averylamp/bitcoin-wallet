...
public class BVerifyServer {
	...
	private static class BVerifyServerImpl extends BVerifyServerAPIImplBase {
		...
		public void getDataRequest(io.grpc.bverify.DataRequest request,
			...
			Set<Receipt> adsData = this.adsManager.getADSData(request.getAdsId().toByteArray(),
					request.getCommitmentNumber());
		...
		public void getAuthPath(io.grpc.bverify.PathRequest request,
			...
			ArrayList<byte[]> keysList = new ArrayList<>();
			for (ByteString keyBytes: request.getAdsIdsList()){
				keysList.add(keyBytes.toByteArray());
			}
			List<byte[]> keys = keysList;
			ArrayList<String> keysStringList = new ArrayList<>();
			for (byte[] keyBytes: keys){
				keysStringList.add(Utils.byteArrayAsHexString(keyBytes));
			}
			List<String> keyStrings  = keysStringList;
